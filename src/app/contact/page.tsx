      ...
      <div className="bg-[#E8DFF5]/30 border-b border-[#E8DFF5] py-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-playfair font-semibold text-[#4A2C5A] text-3xl"
            style={{ fontFamily: "var(--font-playfair)" }}>Contact Us</h1>
        ...
        <div className="grid sm:grid-cols-2 gap-10">
          <div>
            <h2 className="font-playfair font-semibold text-[#4A2C5A] text-xl mb-6"
              style={{ fontFamily: "var(--font-playfair)" }}>Get in Touch</h2>
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} value={form[f.key as keyof typeof form]}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-[#D4C5B0] bg-white text-sm font-inter font-light focus:outline-none focus:ring-2 focus:ring-[#B8A4D4]" />
                </div>
              ))}
              <div>
                <label className="block text-sm font-inter font-light text-[#2A2A2A] mb-1.5">Message</label>
                <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required
                  rows={5} className="w-full px-4 py-3 rounded-xl border border-[#D4C5B0] bg-white text-sm font-inter font-light focus:outline-none focus:ring-2 focus:ring-[#B8A4D4] resize-none" />
              </div>
              <button type="submit" disabled={sending}
                className="w-full py-3.5 bg-[#4A2C5A] text-white font-inter font-normal text-sm rounded-xl hover:bg-[#5B3A6B] transition-colors disabled:opacity-60">
          ...
          <div className="space-y-6">
            <h2 className="font-playfair font-semibold text-[#4A2C5A] text-xl"
              style={{ fontFamily: "var(--font-playfair)" }}>Our Information</h2>
            {[
              ...
              <div key={item.label} className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-[#E8DFF5] flex items-center justify-center shrink-0 text-[#4A2C5A]">
                  {item.icon}
                </div>
                <div>
                  <p className="text-xs font-inter font-medium text-[#8B8B8B] uppercase tracking-wider mb-0.5">{item.label}</p>
                  {item.href ? (
                    <a href={item.href} className="text-sm font-inter font-light text-[#2A2A2A] hover:text-[#4A2C5A] transition-colors">{item.value}</a>
                  ) : (
