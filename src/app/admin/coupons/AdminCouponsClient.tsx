  ...
  return (
    <div className="space-y-5">
      ...
      <button onClick={() => setShowForm(!showForm)}
        className="flex items-center gap-2 px-4 py-2 bg-[#1A1410] text-white text-sm font-inter font-normal rounded-lg hover:bg-[#5B3A6B] transition-colors">
        <Plus size={16} />
        New Coupon
      </button>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 border border-[#F7F3EE]/60">
          <h2 className="font-playfair font-semibold text-[#1A1410] text-xl mb-5"
            style={{ fontFamily: "var(--font-cormorant)" }}>Create Coupon</h2>
          {error && (
          ...
          <form onSubmit={handleCreate} className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })}
                ...
                className="w-full px-4 py-3 rounded-xl border border-[#D4C5B0] bg-[#FAF7F2] text-sm font-inter font-light focus:outline-none focus:ring-2 focus:ring-[#C4992E] transition-all uppercase" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2 flex gap-3">
              <button type="submit" disabled={saving}
                className="px-6 py-2.5 bg-[#1A1410] text-white text-sm font-inter font-normal rounded-xl hover:bg-[#5B3A6B] disabled:opacity-70 flex items-center gap-2 transition-colors">
              ...
              <div key={coupon.id} className="flex items-center justify-between px-6 py-4">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm font-inter font-semibold text-[#1A1410] tracking-wider">{coupon.code}</span>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <button onClick={() => handleToggle(coupon.id, coupon.isActive)}
                    ...
                    className="text-[#8B8B8B] hover:text-[#1A1410] transition-colors disabled:opacity-50">
                    {toggling === coupon.id ? (
