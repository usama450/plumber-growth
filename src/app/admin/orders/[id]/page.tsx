    ...
    <div className="min-h-screen bg-[#FAF7F2]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center gap-2 mb-8 flex-wrap text-sm font-inter font-light">
          <Link href="/admin" className="text-[#8B8B8B] hover:text-[#1A1410]">Dashboard</Link>
          <span className="text-[#D4C5B0]">/</span>
          <Link href="/admin/orders" className="text-[#8B8B8B] hover:text-[#1A1410]">Orders</Link>
          <span className="text-[#D4C5B0]">/</span>
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="bg-white rounded-2xl border border-[#F7F3EE]/60 overflow-hidden">
              <div className="px-6 py-4 border-b border-[#F7F3EE]">
                <h2 className="font-playfair font-semibold text-[#1A1410]"
                  style={{ fontFamily: "var(--font-cormorant)" }}>Items ({order.items.length})</h2>
              </div>
                  ...
                  <div key={item.id} className="flex gap-4 px-6 py-4">
                    {item.productImage ? (
                      <img src={item.productImage} alt={item.productName}
                        className="w-14 h-14 rounded-lg object-cover border border-[#F7F3EE] shrink-0" />
                    ) : (
                      <div className="w-14 h-14 rounded-lg bg-[#F7F3EE]/40 shrink-0" />
                    )}
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-inter font-normal text-[#1A1410]">
            ...
            <div className="bg-white rounded-2xl border border-[#F7F3EE]/60 p-5">
              <h2 className="font-playfair font-semibold text-[#1A1410] mb-3"
                style={{ fontFamily: "var(--font-cormorant)" }}>Customer</h2>
              ...
              <Link href={`/admin/customers?id=${order.user.id}`}
                className="text-xs font-inter font-light text-[#1A1410] hover:underline mt-1 inline-block">
                View customer →
              </Link>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-[#F7F3EE]/60 p-5">
              <h2 className="font-playfair font-semibold text-[#1A1410] mb-4"
                style={{ fontFamily: "var(--font-cormorant)" }}>Summary</h2>
            ...
            {order.stripePaymentIntentId && (
              <div className="bg-white rounded-2xl border border-[#F7F3EE]/60 p-5">
                <h2 className="font-playfair font-semibold text-[#1A1410] mb-2"
                  style={{ fontFamily: "var(--font-cormorant)" }}>Payment</h2>
