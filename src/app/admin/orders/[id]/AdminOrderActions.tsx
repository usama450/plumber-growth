  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <select value={status} onChange={(e) => setStatus(e.target.value)}
        className="px-3 py-2 rounded-lg border border-[#D4C5B0] bg-white text-sm font-inter font-light text-[#2A2A2A] focus:outline-none focus:ring-2 focus:ring-[#B8A4D4]">
        {ALL_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
      </select>
      <button onClick={handleUpdate} disabled={saving || status === currentStatus}
        className="px-4 py-2 bg-[#4A2C5A] text-white text-sm font-inter font-normal rounded-lg hover:bg-[#5B3A6B] transition-colors disabled:opacity-50 flex items-center gap-2">
