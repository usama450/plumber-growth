...
export function CartDrawer() {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-50 bg-[#050507]/60 backdrop-blur-sm transition-opacity duration-300 ${
      ...
      {/* Drawer panel */}
      <div
        className={`fixed top-0 right-0 bottom-0 z-50 w-full sm:w-[420px] bg-[#050507] shadow-2xl flex flex-col transition-transform duration-300 ease-out ${
      ...
      >
        ...
        {items.length > 0 && (
          <div className="px-5 py-3 bg-[#1A0826]/60 border-b border-[#E7D3A8]/10">
            ...
            <div className="h-1.5 bg-[#1A0826] rounded-full overflow-hidden border border-[#E7D3A8]/10">
              <div
                className="h-full bg-[#5A189A] rounded-full transition-all duration-500"
        ...
        {items.length > 0 && (
          <div className="border-t border-[#E7D3A8]/20 px-5 py-4 bg-[#050507]">
            ...
            <Link
              href="/cart"
              onClick={closeCart}
              className="flex items-center justify-center w-full py-3.5 bg-[#5A189A] text-[#F8F4EE] text-[13px] font-medium tracking-wider uppercase rounded border border-[#E7D3A8]/25 hover:bg-[#7B3DBF] hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(90,24,154,0.45)] transition-all duration-200 mb-2"
              style={{ fontFamily: "var(--font-inter)" }}
            >
              View Cart &amp; Checkout
            </Link>
